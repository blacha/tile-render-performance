// Stolen from https://github.com/Esri/quadtree-contours
...
function mipmapReduce({ min, max, width, height, scale }) {
  ...
  function minFinite() {
    ...
    for (let i = 0; i < arguments.length; i++) {
      // significantly faster than the code Babel generates for ...args
      const v = arguments[i];
      if (v < r) r = v;
    }
    return r;
...
class ContourMipmap {
  ...
  min() {
    return this.levels[0].min[0];
  }

  // Get the maximum elevation in the mipmap
  max() {
    return this.levels[0].max[0];
  }
